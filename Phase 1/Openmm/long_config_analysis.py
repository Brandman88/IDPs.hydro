...
print (f'{ParticleN},{round(avg_bondl,3)},{round(avg_lp,3)},{round(avg_Rendsq,3)/100},{round(avg_Rg,3)/10},{round(std_Rg,3)/10},{round(avg_Rgsq,3)/100},{round(np.sqrt(avg_Rgsq),3)/10},{round(avg_Trans_fluctsq,3)}', file = Stat_Output)
Stat_Output.close()

Running_Stats_output = open ("running_stat.dat", "w")
print ("Rg,Rg^2,R_end^2,TransFluctsq", file=Running_Stats_output)
...
for i in range (ntime):
    print(f'{round(np.sqrt(Rgyr2[i]),3)/10},{round(Rgyr2[i],3)/100},{round(RendSq[i],3)/100},{round(TransSq[i],3)}',file=Running_Stats_output)
Running_Stats_output.close()
